...
///
pub fn proxy_remove_edge_where<'a, C, G, F>(g: &'a mut G, f: F)
...
{
	let to_remove = g.all_edges().find(|&e| f(e)).map(|e| (e.source(), e.sink()));
	let proxy =
...
/// Will always need a type annotation for the Constrainer 'C'.
///
pub fn proxy_remove_vertex<'a, C, G>(g: &'a mut G, v: G::Vertex)
	-> Result<G::VertexWeight, ()>
	where
		G: RemoveVertex,
