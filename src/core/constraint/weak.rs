use crate::core::{Graph, EdgeWeighted, Constrainer, Directed, ImplGraph, ImplGraphMut, GraphMut, RemoveVertex, AddEdge, RemoveEdge};
use crate::core::constraint::{proxy_remove_vertex, proxy_remove_edge_where, ConnectedGraph};
use delegate::delegate;
use crate::core::proxy::UndirectedProxy;

///
/// A marker trait for graphs that are weakly connected.
///
/// A graph is weakly connected if, when replacing all edges by undirected versions,
/// there exists a path between every pair of vertices in the graph.
///
/// The distinction between weakly and strongly connected only exists for directed graphs,
/// for undirected ones, they are equal. For this reason, the companion constrainer graph
/// `WeakGraph` only allows directed graphs. For undirected graph, simply use
/// `ConnectedGraph`.
///
...
impl<C: Constrainer> Weak for WeakGraph<C>
	where C::Graph: Graph<Directedness=Directed>
{}
