...
pub trait Subgraph: Graph
{
	...
	/// by sharing a vertex with it, or having an axit edge to one of its vertices.
	///
	fn reaches<G>(&self, other: &G) -> Option<(Self::Vertex, Self::Vertex)>
		where
			G: Subgraph<Vertex=Self::Vertex, VertexWeight=Self::VertexWeight,
						EdgeWeight=Self::EdgeWeight, Directedness=Self::Directedness >,
	{
