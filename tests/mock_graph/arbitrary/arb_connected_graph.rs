use graphene::core::{Directedness, Graph, Constrainer, AddEdge, ImplGraph, ImplGraphMut, Directed, EdgeWeighted};
use graphene::core::constraint::{ConnectedGraph, DirectedGraph, WeakGraph};
use quickcheck::{Arbitrary, Gen};
use crate::mock_graph::{MockGraph, MockEdgeWeight, MockVertex};
use rand::Rng;
...
fn is_connected<D: Directedness>(graph: &MockGraph<D>) -> bool
{
	...
	if let Ok(_) = <DirectedGraph<&MockGraph<D>>>::constrain(graph)
	{
...
///
#[derive(Clone, Debug)]
pub struct ArbWeakGraph(
	pub WeakGraph<MockGraph<Directed>>,
);
...
impl GuidedArbGraph for ArbWeakGraph
{
								...
								-> Self
	{
		...
		// If we are asked to make the singleton or empty graph, we just do
		if v_max <= 2 {
			return Self(WeakGraph::new(
				MockGraph::arbitrary_guided(g,v_min..v_max, e_min..e_max)))
		}
		...
		// We find random vertices in each graph for later use
		let v1 = graph.all_vertices().nth(g.gen_range(0, graph.all_vertices().count())).unwrap();
		let v2 = g2.all_vertices().nth(g.gen_range(0, g2.all_vertices().count())).unwrap();
		...
		// Add edges connecting the two components
		if g.gen_bool(0.5) {
			graph.add_edge_weighted((v1,v_map[&v2], MockEdgeWeight::arbitrary(g))).unwrap();
		} else {
			graph.add_edge_weighted((v_map[&v2],v1, MockEdgeWeight::arbitrary(g))).unwrap();
		}
