...
use crate::mock_graph::arbitrary::{ArbWeakGraph, ArbUnconnectedGraph, ArbTwoVerticesIn, ArbVertexIn, ArbVerticesIn};
use graphene::core::constraint::WeakGraph;
use graphene::core::{Constrainer, Directed, AddEdge, RemoveEdge, Edge, RemoveVertex, NewVertex};
...
///
#[quickcheck]
fn accept_add_edge_weighted(ArbTwoVerticesIn(mut g,v1,v2): ArbTwoVerticesIn<ArbWeakGraph>,
...
///
/// Tests that a WeakGraph accepts removing an edge that isn't critical for weak connectivity.
///
#[quickcheck]
fn accept_remove_edge_where(ArbTwoVerticesIn(mut g,v1,v2): ArbTwoVerticesIn<ArbWeakGraph>,
	e_weight: MockEdgeWeight)
	-> bool
{
...
///
/// Tests that a WeakGraph rejects removing an edge that is critical for weak connectivity.
///
#[quickcheck]
	...
	-> bool
{
	...
	weak.remove_edge_where(|e| (e.source() == v1 && e.sink() == v_map[&v2])).is_err()
}
...
fn accept_remove_vertex(
	...
	v_weight: MockVertexWeight, e_weight: MockEdgeWeight, e_direction: bool)
	-> bool
{
	let v_set = mock.1;
	...
	if e_direction {
		...
		graph.add_edge_weighted((v1, v_new, e_weight.clone())).unwrap();
	}
...
///
/// Tests that a WeakGraph rejects removing a vertex if it renders the graph unconnected
///
#[quickcheck]
	...
	-> bool
{
	...
	// We then connect the two components through a vertex, making it weakly connected
