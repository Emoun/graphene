...
fn contains_vertex(graph: MockGraph<Directed>, v: MockVertex) -> bool
{
	...
	let result_is_some = result.is_ok();
	let result_v = result.map_or(v, |graph| graph.get_vertex().clone());
...
fn new_vertex(mut graph: MockGraph<Directed>, _w: MockVertexWeight) -> bool
{
	...
	let result_v = result.map_or(expected_v, |graph| graph.get_vertex().clone());
